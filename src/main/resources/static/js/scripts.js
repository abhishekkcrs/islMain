...
function openSocket(url = null) {
    ...
    socket.onerror = (error) => {
        ...
        // Show error in detection status
        const status = document.querySelector('.detection-status');
        if (status) {
    ...
    socket.onclose = (event) => {
        ...
        // Show connection closed in detection status
        const status = document.querySelector('.detection-status');
        if (status) {
            status.textContent = 'Connection Closed';
            status.style.color = '#e67e22'; // Orange color for closed
        }
    };
}
...
async function getVideoUrl(word) {
    try {
        ...
        const response = await fetch('https://islbackend.onrender.com/api/lessons');
